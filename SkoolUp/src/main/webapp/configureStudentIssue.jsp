...
<html class="no-js"> 
<head>
  <title>Issue Students Books - SkoolUp</title>
<!-- Favicon -->
...
<script type="text/javascript">
	...
	function retrieveStudents(classID){
		...
		function retrieveStudents1(classID) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					var array = JSON.parse(xmlhttp.responseText);
						...
						var array_element = "<select name='studentID' class='form-control'"+
	...
	function retrieveClassLevel(ClassID, studentID, Genre) {
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				for ( var i = 0; i < array.Release.length; i++) {
					...
					ClassLevel = array.Release[i].level;
				}
				
				retrieveRulesDetails(studentID, Genre, ClassLevel);
			
			}
		};
		xmlhttp.open("GET", "RetrieveClassLevelByClassID?ayclassID="
				+ ClassID, true);
		xmlhttp.setRequestHeader("Content-type", "charset=UTF-8");
		xmlhttp.send();
	}
	...
	function retrieveRulesDetails(studentID, Genre, ClassLevel) {
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				for ( var i = 0; i < array.Release.length; i++) {
					console.log("inside: "+array.Release[i].bookCount);
				...
				if(check == 1){
					...
					retrieveStudentsBookIssueCount(studentID, BookCount);
				}
			}
		};
		xmlhttp.open("GET", "RetrieveRulesDetailsByClassLevel?level="
				+ ClassLevel+"&genre=" + Genre, true);
		xmlhttp.setRequestHeader("Content-type", "charset=UTF-8");
		xmlhttp.send();
	}
	
	function retrieveStudentsBookIssueCount(studentID, BookCount) {
		...
		xmlhttp.open("GET", "RetrieveStudentsBookIssueCount?studentID="
				+ studentID  +"&bookCount=" + BookCount, true);
		xmlhttp.setRequestHeader("Content-type", "charset=UTF-8");
		xmlhttp.send();
	}
	</script>
	...
	<script type="text/javascript" charset="UTF-8">
		...
		function retrieveIssuedBooks(studentID, ClassID) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					for ( var i = 0; i < array.Release.length; i++) {
						 ...
						 studentIssueID = array.Release[i].studentIssueID;
						...
						tableTag += "<td align='center'><a href='javascript:returnBook(\"" + studentIssueID + "\", \"" + BookID + "\");'>Return</a></td>";
						tableTag += "</tr></tbody>";
					...
					if(check == 0){
						...
						retrieveClassLevel(ClassID, studentID, Genre);
					}
				}
			};
			xmlhttp.open("GET", "RetrieveIssuedBooks?studentID="
					+ studentID, true);
			xmlhttp.setRequestHeader("Content-type", "charset=UTF-8");
			xmlhttp.send();
		}
...
<script type="text/javascript">
	...
	function returnBook(studentIssueID, BookID) {
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				trTag += "<tr style='font-size: 15px; '><td >Book Name </td><td > : </td></b><td style='padding-left: 20px;'><input type='hidden' id='returnbookIssueID' name='bookID' value='"+BookID+"' ><input type='hidden' id='returnStudentIssueID' name='studentIssueID' value='"+studentIssueID+"' >"+BookName+"</td></tr>"
				...
				+"<tr style='font-size: 15px; '><td style='padding-top: 20px;'><div class='row'><div class='col-md-4'>Fine</div><div class='col-md-6' ><span style='font-size: 17px;'>( &#8377 )</span></div></div> </td><td style='padding-top: 20px;'> : </td><td style='padding-left: 20px;padding-top: 20px;'><input type='number' id='FineID' name='fineAmount'  class='form-control' style='width: 75px;' readonly='readonly' value='"+Fine+"' </td></tr>";
		...
		xmlhttp.open("GET", "RetrieveStudentDetailsForBookReturn?studentIssueID="
				+ studentIssueID, true);
		xmlhttp.setRequestHeader("Content-type", "charset=UTF-8");
		xmlhttp.send();
	}
...
<script type="text/javascript">
	function showIssueBook(classID, studentID){
...
<script type="text/javascript">
	...
	function retrieveBooks(ScansValue, ClassID, studentID ) {
...
<form id="loadStudentFOrmNew" action="ReturnBook" method="POST" onsubmit="submitForm('loadStudentFOrmNew', 'submitID', 'ReturnBook');" data-validate="parsley" enctype="multipart/form-data" class="form parsley-form">
  ...
  <div class="content" style="min-height: 0px;">
    ...
    <div class="content-container" style="z-index: 0;">
      ...
      <div class="content-header">
        <h2 class="content-header-title">Issue Books - Students</h2>
        <ol class="breadcrumb">
          <li><a onclick="windowOpen();">Home</a></li>
          <li class="active">Issue Books - Students</li>
        </ol>
      </div> <!-- /.content-header -->
          ...
          <div class="portlet">
            ...
            <div class="portlet-content"> 
	           ...
	           <div class ="row">   
                	...
                	<div class ="col-md-1">
	                 	<label for="author">Class :<span class="required">*</span></label>
	                </div>
	                 <div class ="col-md-2">
	                    <s:select list="StandardDivisionList" id="classID" required="required" class="form-control" value="-1" headerKey="-1" 
	                    	headerValue="Select Class" name="ayclassID" onchange="retrieveStudents(this.value);"></s:select>
	                </div>
	             	
	          		<div class="col-md-2" id ="studentdivID" style="width: 105px;">
		              <label for="Student">Student :</label>
		            </div>
		            <div class ="col-md-3">
		              <select name="studentID" id="StudID" class="form-control" onchange="retrieveIssuedBooks(this.value, classID.value);">
					      <option value="-1" >Select Student</option>
					  </select>
				    </div>
			        
	               <div class ="col-md-2" id ="updateID" style="width: 12%;">
					  <button type="button" class="btn btn-success" id="showIssueBookID" onclick="showIssueBook(classID.value, StudID.value);">Issue Book</button>
	               </div> 
	               ...
	               <div class ="col-md-1" id="TableDiv1ID" style="display: none;width: 20%;" >
	                <input type="text" name="bookScanValue" id="scanValueID" required="required" class="form-control" onchange="retrieveBooks(this.value, classID.value, StudID.value);">
              ...
              <form id="loadStudentFOrm" action="IssueStudentBooks" method="POST" onsubmit="submitForm('loadStudentFOrm', 'submitID', 'IssueStudentBooks');" data-validate="parsley" class="form parsley-form">
                 ...
                 <div class="row" id="BookDetailsIssueID" style="display: none;">
					...
					<div class ="col-md-3" >
						...
						<input type="hidden" id="studentBookID" name="bookID">
						<input type="hidden" name="studentID" id="studentHiddenID">
		            ...
		            <div class ="col-md-3" >
	                 	...
	                 	<div class="input-group date ui-datepicker ">
				           <input type="text" name="expectedReturnDate" id="expectedDateID" class="form-control" placeholder="Expected Return Date(dd/mm/yyyy)" data-required="true">
				           <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
				        </div>
		            </div>  	
