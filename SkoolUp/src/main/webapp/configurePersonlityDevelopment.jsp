...
<%
    	...
    	String Term = daoInf.retrieveTerm(AcademicYearID);
		...
		String generateGrades = (String)(request.getAttribute("generatedGrades"));
		if(generateGrades == null || generateGrades == ""){
			generateGrades = "dummy";
		}
		
		String PersonalityGradeCheck = (String)(request.getAttribute("PersonalityGradeCheck"));
		if(PersonalityGradeCheck == null || PersonalityGradeCheck == ""){
			PersonalityGradeCheck = "dummy";
		}
	
    	List<ConfigurationForm> studentAssessmentList = (List<ConfigurationForm>) request.getAttribute("studentAssessmentList");
    %>
    
  <title><% if(loginform.getMedium().equals("mr")){ %>व्यक्तिमत्व विकास विषय कॉन्फीग्युर करा  - SkoolUp<% }else{ %>Configure Personality Development Subject - SkoolUp <% } %></title>
<!-- Favicon -->
     ...
     /* Load student standard and division ID */ 
      ...
      function checkStandard(sid, subject, exam, subID){
    	 ...
    	 if(sid == "-1"){
        	  ...
        	  $("#loadStudentFOrm").attr("action","LoadStudentsForPersonlityDevelopment");
      ...
      function checkValues(sid, subject, subID){
     	 ...
     	 if(sid == "-1"){
    	    	...
    	    	$(".navbar").css("opacity","0.1"); 
     			 ...
     			 $("#loadStudentFOrm").attr("action","GenerateGradesForPersonalityDevelopment");
     	    	 $("#loadStudentFOrm").submit();
      ...
      function downloadExcel(sid){
    	...
    	if(sid == "-1"){
   			...
   			document.location="StudentsPersonalityReportExcel";
   		}
   	}
      ...
      function submitForm(subID){
    	  ...
    	  $("#loadStudentFOrm").attr("action","AddStudentAssessmentForPersonalityDevelopment");
    	  $("#loadStudentFOrm").submit();
...
function retrieveSubject(examID, standardValue){
	...
	function retrieveSubject1(examID,standardValue){
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				var array_element = "<select name='subjectID' id='editSubDivID' class='form-control' onchange='CheckFetchOldGradeButton(this.value, \""+standardValue+"\",\""+examID+"\");'"+
		...
		xmlhttp.open("GET", "RetrieveSubjectListByUserIDForPersonalityDevelopment?ExaminationID="
	...
	function CheckFetchOldGradeButton(subjectID, standardValue, examID){
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				...
				if(check == 0){
					...
					$("#fetchOldBtnID").attr("disabled",false);
					
				}else{
					
					$("#fetchOldBtnID").attr("disabled",true);	
					
				}
			}
		};
		xmlhttp.open("GET", "VerifySubjectAssessmentIDPresent?ExaminationID="
				+ examID+"&standardDivName="+standardValue+"&subjectID="+subjectID, true);
		xmlhttp.setRequestHeader("Content-type", "charset=UTF-8");
		xmlhttp.send();
	}
 ...
 function disable1(AbsentID, MarksObtainedID, GradeBasedID, HiddenInputID, GradeBasedHiddenID, GradeBasedHiddenName){
	 //alert(AbsentID+".."+HiddenInputID);
	    if (AbsentID == "0") {
	    	
	    	$("#"+MarksObtainedID).attr("readonly",false);
	    	$("#"+GradeBasedID).attr("disabled",false);
	    	
	    	$("#"+GradeBasedHiddenID).removeAttr('name')
	    	
	    	$("#"+HiddenInputID).val(AbsentID);
	    } else {
			$("#"+MarksObtainedID).attr("readonly",true);
			$("#"+GradeBasedID).attr("disabled",true);
			
			$("#"+GradeBasedHiddenID).attr('name', GradeBasedHiddenName);
	...
	<div class="content-header">
      ...
      <div class="row">
          ...
          <div class="portlet">
            ...
            <div class="portlet-header">
              ...
              <h3>
                ...
                <% if(loginform.getMedium().equals("mr")){ %>व्यक्तिमत्व विकास  विषय माहिती  <% }else{ %> Personality Development & Life Skills Subject Details <% } %>
              </h3>
			...
			<div style="margin-top:15px;">
              ...
              <form id="loadStudentFOrm" action="LoadStudentsForPersonlityDevelopment" method="POST" data-validate="parsley"  class="form parsley-form">
				
				<input type="hidden" class="form-control" id= "valueID" name="value" value="Personality Development & Life Skills" >
				...
				<% if(loginform.getMedium().equals("mr")){ %>
					...
					<div class="col-md-2">
						<s:select list="StandardListForPersonalityDevelopment" onchange="retrieveSubject(examID.value, this.value);" class="form-control" headerKey="-1" id="SID" headerValue="इयत्ता निवडा" name="standardDivName" ></s:select>
					</div>
				
		            <div class="col-md-2" id="editSubjectDivID">
		            	<s:select list="SubjectListForPersonalityDevelopment" class="form-control" headerKey="0" id="editSubDivID"  headerValue="विषय निवडा" name="subjectID" onchange="CheckFetchOldGradeButton(this.value, SID.value, examID.value);" ></s:select>
		            </div>
					
					<div class="col-md-2">
						<button type="button" class="btn btn-warning" id="subID" onclick="return checkStandard(SID.value, editSubDivID.value, examID.value, 'subID');">विद्यार्थी पहा</button>
					</div>
					
					<%if(Term.equals("Term II")){
						
						if(PersonalityGradeCheck.equals("Found")){%>
					
						<div class="col-md-2">
							<button type="button" class="btn btn-success" id="fetchOldBtnID" id="checkSubID" disabled="disabled" onclick="return checkValues(SID.value, editSubDivID.value, 'checkSubID');">जुने ग्रेड फेच करा</button>
						</div>
						<%}else{ %>
						<div class="col-md-2">
							<button type="button" class="btn btn-success" id="fetchOldBtnID" onclick="return checkValues(SID.value, editSubDivID.value);">जुने ग्रेड फेच करा</button>
						</div>
		...
		<% }else{ %>
					...
					<div class="col-md-2">
						<s:select list="StandardListForPersonalityDevelopment" onchange="retrieveSubject(examID.value, this.value);" class="form-control" headerKey="-1" id="SID" headerValue="Select Standard" name="standardDivName" ></s:select>
					</div>
				
		            <div class="col-md-2" id="editSubjectDivID">
		            	<s:select list="SubjectListForPersonalityDevelopment" class="form-control" headerKey="0" id="editSubDivID"  headerValue="Select Subject" name="subjectID" onchange="CheckFetchOldGradeButton(this.value, SID.value, examID.value);" ></s:select>
		            </div>
					
					<div class="col-md-2">
						<button type="button" class="btn btn-warning" id="subID" onclick="return checkStandard(SID.value, editSubDivID.value, examID.value, 'subID');">Load Students</button>
					</div>
					...
					<%if(Term.equals("Term II")){
						...
						<div class="col-md-2">
							<button type="button" class="btn btn-success" id="fetchOldBtnID" id="checkSubID" disabled="disabled" onclick="return checkValues(SID.value, editSubDivID.value, 'checkSubID');">Fetch Old Grades</button>
						</div>
						<%}else{ %>
						<div class="col-md-2">
							<button type="button" class="btn btn-success" id="fetchOldBtnID" onclick="return checkValues(SID.value, editSubDivID.value);">Fetch Old Grades</button>
						</div>
		              ...
		              <table id="" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
	                    ...
	                    <s:iterator value="examinatrionList" >
		                   	...
		                   	<%}else{ %>
	         					...
	         					<%-- <td><input type="text" class="form-control" id="gradeBasedID<s:property value="studentID"/>" readonly="readonly" name="" > </td> --%>
		                   ...
		                   <%
				                ...
				                if(generateGrades.equals("Yes")){  
				                	System.out.println("yes");
				                %>
				                		 
				              	<tr id="StudentTRID<%=form.getStudentID() %>">
		                   			<td><%=form.getRollNumber() %><input type="hidden" name="studID" value="<%=form.getStudentID() %>"><input type="hidden" name="editRegsitrationID" value="<%=form.getRegistrationID() %>"> </td>
			                   		...
			                   		<input type="hidden" name="newGradeBase" value="<%=form.getGradeBased()%>">
			                   		<input type="hidden" name=newSubjAssmntID value="<%=form.getSubjectAssessmentID() %>">
			                   		</td>
		                   			<td>
		                   		<div>
		                   			<%
		                   			...
		                   			onchange="disable1(this.value, 'edidmarksObtainedID<%=form.getStudentID() %>','editgradeBasedID<%=form.getStudentID() %>', 'absntFlagID<%=form.getStudentID() %>','editgradeBasedHiddenID<%=form.getStudentID() %>','gradeObtainedArr');" 
		                   			 />Yes
		    						<input type="radio" name="name<%=form.getStudentID() %>"  value="0" 
		    						onchange="disable1(this.value, 'edidmarksObtainedID<%=form.getStudentID() %>','editgradeBasedID<%=form.getStudentID() %>', 'absntFlagID<%=form.getStudentID() %>', 'editgradeBasedHiddenID<%=form.getStudentID() %>','gradeObtainedArr');"  />No
		                   			<%
		                   				}else{
		                   			%>
		                   			<input type="radio" name="name<%=form.getStudentID() %>" value="1"
		                   			onchange="disable1(this.value, 'edidmarksObtainedID<%=form.getStudentID() %>','editgradeBasedID<%=form.getStudentID() %>', 'absntFlagID<%=form.getStudentID() %>','editgradeBasedHiddenID<%=form.getStudentID() %>','gradeObtainedArr');"   />Yes
		    						<input type="radio" name="name<%=form.getStudentID() %>" checked="checked" value="0" 
		    						onchange="disable1(this.value, 'edidmarksObtainedID<%=form.getStudentID() %>','editgradeBasedID<%=form.getStudentID() %>', 'absntFlagID<%=form.getStudentID() %>','editgradeBasedHiddenID<%=form.getStudentID() %>','gradeObtainedArr');"  />No
		    						<%
		                   				}
		    						%>
		    						<input type="hidden" id="absntFlagID<%=form.getStudentID() %>" name="newAbsentFlag" value="<%=form.getAbsentFlag()%>">
		                   		</div>
		                   	</td>
		                   		<% if(GradeBased == 1){ %>
		                   			...
		                   			<% if(SubjectType == "1"){
			                   			if(form.getAbsentFlag() == 1){	%>
			                   			<td>
			                   				<input type="hidden" name="marksObtainedArr" value="<%=form.getMarksObtained() %>" >
			                   				<input type="hidden" name="newTotalMarksID" value="<%=form.getTotalMarks()%>">
			                   				<input type="hidden" name="gradeObtainedArr" id="editgradeBasedHiddenID<%=form.getStudentID() %>" value="">
			                   				<select class="form-control" disabled="disabled" id="editgradeBasedID<%=form.getStudentID() %>" name="gradeObtainedArr" >
				                   			<option value="">Select Grade</option>
			                   		...
			                   		<td>
			                   				<input type="hidden" name="marksObtainedArr" value="<%=form.getMarksObtained() %>" >
			                   				<input type="hidden" name="newTotalMarksID" value="<%=form.getTotalMarks()%>">
			                   			<select class="form-control" id="editgradeBasedID<%=form.getStudentID() %>" name="gradeObtainedArr" >
				                   			<option value="">Select Grade</option>
		                   		...
		                   		<%}else{
		                   			if(form.getAbsentFlag() == 1){	
		                   		%>
		                   		<td><input type="number" class="form-control" readonly="readonly" name="marksObtainedArr" id="edidmarksObtainedID<%=form.getStudentID() %>" value="<%=form.getMarksObtained() %>"  > </td>
		                   		<%
		                   			}else{
		                   		%>
		                   		<td><input type="number" class="form-control" name="marksObtainedArr" id="edidmarksObtainedID<%=form.getStudentID() %>" value="<%=form.getMarksObtained() %>"  > </td>
		                   		<%
		                   		}
		                   		%>
		                   		
		                   		<td><%=form.getTotalMarks() %> <input type="hidden" name="newTotalMarksID" value="<%=form.getTotalMarks()%>">
		                   		<input type="hidden" class="form-control" name="gradeObtainedArr" id="editgradeBasedID<%=form.getStudentID() %>" readonly="readonly" value="<%=form.getGrade() %>" >
		                   		</td>
				           ...
				           <% }else{
		                   	...
		                   	<tr id="StudentTRID<%=form.getStudentID() %>">
		                   		...
		                   		<div>
		                   			...
		                   			%>
		                   			<input type="radio" name="name<%=form.getStudentID() %>"  checked="checked" value="1" 
		                   			onchange="disable1(this.value, 'edidmarksObtainedID<%=form.getStudentID() %>','editgradeBasedID<%=form.getStudentID() %>', 'absntFlagID<%=form.getStudentID() %>','editgradeBasedHiddenID<%=form.getStudentID() %>','editGradeObtainedArr');" 
		                   			 />Yes
		    						<input type="radio" name="name<%=form.getStudentID() %>"  value="0" 
		    						onchange="disable1(this.value, 'edidmarksObtainedID<%=form.getStudentID() %>','editgradeBasedID<%=form.getStudentID() %>', 'absntFlagID<%=form.getStudentID() %>','editgradeBasedHiddenID<%=form.getStudentID() %>','editGradeObtainedArr');"  />No
		                   			<%
		                   				}else{
		                   			%>
		                   			<input type="radio" name="name<%=form.getStudentID() %>" value="1"
		                   			onchange="disable1(this.value, 'edidmarksObtainedID<%=form.getStudentID() %>','editgradeBasedID<%=form.getStudentID() %>', 'absntFlagID<%=form.getStudentID() %>','editgradeBasedHiddenID<%=form.getStudentID() %>','editGradeObtainedArr');"   />Yes
		    						<input type="radio" name="name<%=form.getStudentID() %>" checked="checked" value="0" 
		    						onchange="disable1(this.value, 'edidmarksObtainedID<%=form.getStudentID() %>','editgradeBasedID<%=form.getStudentID() %>', 'absntFlagID<%=form.getStudentID() %>', 'editgradeBasedHiddenID<%=form.getStudentID() %>','editGradeObtainedArr');"  />No
		    						<%
		                   				}
		                   		...
		                   		<% if(GradeBased == 1){ %>
			                   		...
			                   		<td>
			                   			...
			                   			<input type="hidden" name="editGradeObtainedArr" id="editgradeBasedHiddenID<%=form.getStudentID() %>" value="">
			                   			<select class="form-control" disabled="disabled" id="editgradeBasedID<%=form.getStudentID() %>" name="editGradeObtainedArr" >
				                   			<option value="">Select Grade</option>
		                   		...
		                   		<%-- <td><input type="text" class="form-control" name="editGradeObtainedArr" value="<s:property value="grade"/>"> </td> --%>
		                   		...
		                   		<%-- <td><input type="text" class="form-control" name="editGradeObtainedArr" id="editgradeBasedID<%=form.getStudentID() %>" readonly="readonly" value="<%=form.getGrade() %>" > </td> --%>
