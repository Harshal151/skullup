...
<html class="no-js"> 
<head>
  <title>Report-Card History - SkoolUp</title>
<!-- Favicon -->
...
<script type="text/javascript">
 ...
 <script type="text/javascript">
      ...
      function windowOpen1(){
          document.location="reportCardHistory.jsp";
        }
   
      function checkStandard(academicYearID, term){
    	if(academicYearID == "-1" ){
    		...
    		return false;
  			...
  			$("#validate-basic").attr("action","LoadClassStudentHistoryReport");
...
<script type="text/javascript">
	...
	if (window.XMLHttpRequest) {
		...
		function retrieveStandard(academicYearID){
			...
			if(academicYearID == "-1"){
				...
				retrieveStandard1(academicYearID);
			}
			
		}

		function retrieveStandard1(academicYearID) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					var array = JSON.parse(xmlhttp.responseText);
						...
						var array_element = "<select name='standardID' id='' class='form-control'"+
						"> <option value='-1'>Select Standard</option></select>";
						...
						for ( var i = 0; i < array.Release.length; i++) {
							...
							array_element += "<option value='"+array.Release[i].standardID+"'>"+array.Release[i].standard+"</option>";
						}
			...
			xmlhttp.open("GET", "RetrieveStandardListForacademicYear?academicYearID="
					+ academicYearID, true);
	...
	function retrieveDivision(standardID){
		...
		if(standardID == "-1"){
			...
			document.getElementById("DivID").innerHTML = array_element;
...
<script type="text/javascript">
	...
	function submitReportForm1(){
		$("#validate-basic").attr("action","StudentsHistoryCustomReportExcel");
		$("#validate-basic").submit();
	}
</script>
  ...
  <div class="content">
    ...
    <div class="content-container" style="z-index: 0;">
       ...
       <div class="content-header">
        <h2 class="content-header-title">Report-Card History</h2>
        <ol class="breadcrumb">
       ...
       <div class="row">
        ...
        <div class="col-sm-12">
          ...
          <div class="portlet">
		   ...
		   %> 
		  	<form id="validate-basic" action="LoadClassStudentHistoryReport" method="POST" data-validate="parsley" class="form parsley-form">
				
                <div class="row">
               
               		<div class="col-md-2">
						<s:select list="InActiveAcademicYearList" class="form-control" headerKey="-1" id="inActiveID" onchange="retrieveStandard(this.value)" headerValue="Select Academic Year" name="academicYearID"></s:select>
					</div>
					...
					<% if(loadStudentSearch.equals("Enabled")){ %>
							<s:select list="StandardList" class="form-control" headerKey="-1" id="stdDivID" headerValue="Select Standard" name="standardID" onchange="retrieveDivision(this.value,inActiveID.value);"></s:select>
					<% }else{ %>
						<select name="standardID" id="stdDivID" class="form-control" onchange="retrieveDivision(this.value);">
					...
					<div class="col-md-2">
						<% if(loadStudentSearch.equals("Enabled")){ %>
							<s:select list="DivisionList" class="form-control" headerKey="-1" id="DivID" headerValue="Select Division" name="divisionID"></s:select>
						<% }else{ %>
						<select name="divisionID" id="DivID" class="form-control">
				            <option value="-1">Select Division</option>
				        </select>
				        <%} %>
				     </div>
					...
					<div class="col-md-2">
						<button type="submit" class="btn btn-warning" onclick="return checkStandard(inActiveID.value, termID.value)">Load Students</button>
					</div>
				...
				%>
					<div class="col-md-2" align="left">
						<button class="btn btn-success" type="button" onclick="submitReportForm1();">Excel Report</button>
				  	</div>
				<% } %> 
				</div>
		          ...
		          <table id="" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
	                  ...
	                  <tbody>
				       ...
				       <tr id="newTRID<%=form.getStudentID()%><%=form.getAYClassID()%>" style="font-size: 14px;width: 6%;">
				       		...
				       		<td>
				       			<a href="ConfigureHistoryReport?studentID=<%=form.getStudentID()%>&term=<%=form.getTerm()%>&standardID=<%=form.getStandardID()%>&AYClassID=<%=form.getAYClassID()%>&academicYearID=<%=form.getAcademicYearID()%>&registrationID=<%=form.getRegistrationID()%>&result=<%=form.getResult()%>">Report</a>
				       		</td>
				       </tr> 
